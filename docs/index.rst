...
This code runs on Python 2.6 and 2.7. To install, I recommend using the Python package manager `pip <https://pypi.python.org/pypi/pip?>`_:

.. code-block:: bash
   
   sudo pip install pyIAST

Alternatively, clone the repository on `Github <https://github.com/CorySimon/pyIAST>`_. `cd` into the main directory `pyIAST` and run the `setup.py` script in the terminal:
